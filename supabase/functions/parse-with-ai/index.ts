...
serve(async (req) => {
  ...
  try {
    // --- AUTH: Verify JWT, reject anonymous/unauthenticated callers ---
    ...
    // Auth passed — user is authenticated. Cross-tenant is prevented because
    // the caller can only access their own client data via RLS on the client side.
